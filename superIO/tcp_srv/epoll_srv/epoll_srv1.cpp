...
class TcpSocket{
    ...
    public:
        ...
        bool Socket(){
            ...
            int option = 1;
            if(setsockopt(_sockfd, SOL_SOCKET, SO_REUSEADDR, (void*)&option, sizeof(int)) < 0){
        ...
        ssize_t Recv(char *buf, size_t len){
            ...
            int ret = 0;
            // size_t 是一个无符号类型的数据
            // 当无符号数据与有符号数据进行比较的时候，是按照无符号数进行比较的
            // 这种比较的判断方法是：两个相减看是否大于0 或者 小于0
            // 但是无符号数据进行相减之后的结果是不会小于0的，因为结果依然是一个无符号数据
            // 所以会导致判断出问题
            while(rlen < (int)len){
